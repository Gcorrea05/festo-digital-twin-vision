...
const toMinuteIsoUTC = (d: Date) =>
  new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), d.getUTCMinutes(), 0, 0)).toISOString();
...
type MpuPoint = { ts: string; ax: number; ay: number; az: number; gx: number; gy: number; gz: number };
...
const Analytics: React.FC = () => {
  ...
  /* ---------- Seletor de gráfico em cada guia ---------- */
  ...
  /* ---------- NOVO: dataset CPM×Runtime via endpoint dedicado ---------- */
  ...
  const cpmRtMerged = useMemo(() => {
    ...
    for (const r of dataAgg) map.set(r.minute, { cpm: typeof r.cpm === "number" ? r.cpm : 0, runtime: r.runtime_s ?? 0 });
      ...
      <div className="mx-auto w-full max-w-screen-2xl px-6 md:px-8 pb-12">
        ...
        <header className="mb-6">
          ...
          <p className="page-subtitle">
            Tendências, agregados e comparativos de desempenho dos atuadores.
          </p>
        </header>
        ...
        <Card>
          <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            {/* Toggle Modelo 1 / 2 — mesmo estilo das outras páginas */}
            <div className="inline-flex rounded-2xl bg-black/30 p-1 border border-white/10">
              <button
                className={`px-4 py-2 text-base rounded-xl transition ${
                  act === 1 ? "bg-cyan-600 text-white" : "bg-transparent text-white/80 hover:text-white"
                }`}
                onClick={() => setAct(1)}
              >
                Modelo 1
              ...
              <button
                className={`px-4 py-2 text-base rounded-xl transition ${
                  act === 2 ? "bg-cyan-600 text-white" : "bg-transparent text-white/80 hover:text-white"
                }`}
                onClick={() => setAct(2)}
              >
                Modelo 2
          ...
          <CardContent>
            ...
            <Tabs defaultValue="producao">
              <TabsList className="flex overflow-x-auto gap-2 sm:grid sm:grid-cols-3 rounded-md p-1 bg-muted/20 w-full no-scrollbar">
